...
\begin{answers}
    ...
    \good{Si $E$ est un ensemble fini, $\Card(A\Delta B) \le \Card A+ \Card B$.}

    \bad{Si $E$ est un ensemble fini, $\Card(A\Delta B) < \Card A+ \Card B$.}
\end{answers}
\end{question}
...
\begin{answers}
    \bad{Si $A \cap B = \emptyset$ alors $\Card \mathcal{S} = p+q$.}

    \good{Si $A \cap B = \emptyset$ alors $\Card \mathcal{S} = pq$.}

    \bad{Si $A \subset B$ alors $\mathcal{S} = \emptyset$.}

    \bad{$\Card \mathcal{T} = C_n^p \times r$.}

    \good{$\Card \mathcal{T} = C_p^r \times q$.}
\end{answers}
\end{question}
...
\begin{answers}
    \good{$ab = \pgcd(a,b)\times \ppcm(a,b)$.}

    \bad{$abc = \pgcd(a,b,c)\times \ppcm(a,b,c)$.}

    \good{$\ppcm(a,b,c)$ est divisible par $c$.}

    \bad{$\ppcm(1932,345)= 19320$.}
...
\begin{answers}
    ...
    \good{Si $a = bq + r$ est la division euclidienne de $a$ par $b$ alors $\pgcd(a,b) = \pgcd (b,r)$.}

    \good{Il existe $u,v \in \Zz$ tels que $195u+2380v=5$.}

    \bad{Sachant qu'il existe $u,v$ tels que $2431u+65520v = 39$
alors $\pgcd(2431,65520)=39$.}
...
\begin{answers}
    ...
    \bad{Dans $\Zz[X]$, $\pgcd(X(X-1)^2(X^2+1) ,X^2(X-1)(X^2-1) ) = X(X-1)$.}

    \good{Dans $\Zz[X]$, $\pgcd(X^4+X^3+X^2+X,X^3-X^2-X+1) = X+1$.}
\end{answers}
\end{question}
...
\begin{question}
Suites définies par récurrence. Soit $f(x) = 2x(1-x)$ et la suite
définie par $u_0 \in [0,1]$ et $u_{n+1} = f(u_n)$.
\begin{answers}
...
\begin{answers}
    ...
    \good{$\forall x \in \Rr \  \ch x \ge \sh x$.}
    \good{$\frac{\ch x}{\sh x}$ tend vers $1$ quand $x$ tend vers $+\infty$.}
    \good{$\ch 2x = 1 + 2\sh^2 x$.}
    \bad{$\tanh(a+b) = \frac{\tanh a+ \tanh b}{1- \tanh a \tanh b}$.}
\end{answers}
\end{question}
...
\begin{answers}
    ...
    \bad{$\Arcsin (\sin x) = x$ pour tout $x \in [0, 2\pi[$.}
    \bad{Si $f(x) = \Arctan (x^2)$ alors $f'(x) = \frac{1}{1+x^4}$.}
\end{answers}
\end{question}

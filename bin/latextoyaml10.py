...
parser = argparse.ArgumentParser(
    description="Conversion of mutliple choice questions from LaTeX to the yaml format."
)
...
def add_space(text):
    ...
    )  # Delete potential space at the beginning
    # print('After\n'+text)
    spacetext = indent + spacetext  # First line
    spacetext = re.sub(
        "\n", "\n" + indent, spacetext, flags=re.MULTILINE
    )  # Add indentation
    return spacetext
...
def dollars_to_tags(text):
    ...
    # Substitute $$ to \[ \]
    text2 = re.sub(
        "\\$\\$(.+?)\\$\\$", "\\\\[\\g<1>\\\\]", text, flags=re.MULTILINE | re.DOTALL
    )
    # Substitute $ to \( \)
    text1 = re.sub(
        "\\$(.+?)\\$", "\\\\(\\g<1>\\\\)", text2, flags=re.MULTILINE | re.DOTALL
    )
    return text1
...
def one_exo_to_yaml(text_exo, qcmdict={}):
    ...
    """
    # suppPercent=re.compile((.*?(?![\\]).%))
    # delete the comments at the end of a line after a '%', except for \% !!
    # fisrt the ligne begins by %
    text_exo = re.sub(r"^%.*$", "", text_exo, flags=re.MULTILINE)
    # then the end of the ligne after the first %
    text_exo = re.sub(r"(.*?(?![\\\\]).%).*$", r"\1", text_exo, flags=re.MULTILINE)
    # At last all the remaining %
    text_exo = re.sub(r"([^\\\\])%", r"\1", text_exo, flags=re.MULTILINE)

    # text_exo = re.sub("[^\\\\]%", "", text_exo) this doesn't work
    ...
    theallquestion = re.search(
        "(.*?)(?=[\\s*]\\\\begin{answers})", text_exo, flags=re.MULTILINE | re.DOTALL
    )
    ...
    thetitle = re.search(
        "^(\\s*)\\[(.*?)\\]", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if thetitle:
    ...
    thenum = re.search(
        "(?<=\\\\qnum\\{)(.*?)(?=\\})", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if thenum:
    ...
    theid = re.search(
        "(?<=\\\\qid\\{)(.*?)(?=\\})", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if theid:
        myid = theid.group(0)
        # print('\n\n---id---\n'+myid+'\n\n')
    ...
    # the author : idem
    theauthor = re.search(
        "(?<=\\\\qauthor\\{)(.*?)(?=\\})", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if theauthor:
    ...
    # the section : idem
    thesection = re.search(
        "(?<=\\\\qsection\\{)(.*?)(?=\\})", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if thesection:
    ...
    # the tags : idem
    thetags = re.search(
        "(?<=\\\\qtags\\{)(.*?)(?=\\})", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if thetags:
    ...
    # the type : idem
    thetype = re.search(
        "(?<=\\\\qtype\\{)(.*?)(?=\\})", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if thetype:
    ...
    if thetolerance:
        ...
        # print('\n\n---tolerance---\n'+mytolerance+'\n\n')
        allquestion = re.sub(
            "\\\\qtolerance\\{(.*?)\\}", "", allquestion, flags=re.MULTILINE | re.DOTALL
        )
    else:
        mytolerance = None
    ...
    if theoneline:
        oneline = True
        allquestion = re.sub(
            "\\\\qoneline", "", allquestion, flags=re.MULTILINE | re.DOTALL
        )
    else:
        oneline = False
    # the multicol flag : idem
    themulticol = re.search(
        "\\\\qmulticols", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if themulticol:
        multicol = True
        allquestion = re.sub(
    ...
    thekeeporder = re.search(
        "\\\\qkeeporder", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if thekeeporder:
        keeporder = True
        allquestion = re.sub(
    ...
    theidontknow = re.search(
        "\\\\qidontknow", allquestion, flags=re.MULTILINE | re.DOTALL
    )
    if theidontknow:
        idontknow = True
        allquestion = re.sub(
    ...
    # Find the answers
    theanswers = re.search(
        "(?<=\\\\begin{answers})(.*)(?=[\\s*]\\\\end{answers})",
        text_exo,
        flags=re.MULTILINE | re.DOTALL,
    )
    ...
    while i < n:
        ...
        ans = re.search(
            "(?<=\\{)(.*)(?=\\})", theans, flags=re.MULTILINE | re.DOTALL
        ).group(0)
        ...
        if feedback is not None:
            ...
            thefeedback = re.sub(
                "\\s+$", "", thefeedback, flags=re.MULTILINE | re.DOTALL
            )
        else:
            if score is not None:
    ...
    theexplanations = re.search(
        "(?<=\\\\begin{explanations})(.*)(?=[\\s*]\\\\end{explanations})",
        text_exo,
        flags=re.MULTILINE | re.DOTALL,
    )
    if theexplanations:
...
qcmtitle = ""
qcmauthor = ""
theqcmtitle = re.search(
    "(?<=\\\\qcmtitle\\{)(.*?)(?=\\})", text_all, flags=re.MULTILINE | re.DOTALL
)
theqcmauthor = re.search(
    "(?<=\\\\qcmauthor\\{)(.*?)(?=\\})", text_all, flags=re.MULTILINE | re.DOTALL
)
...
text_split_section = re.split(
    "(\\\\section\\{(.*?)\\})", text_all, flags=re.MULTILINE | re.DOTALL
)


text_split = []
for text_section in text_split_section:
    text_split_subsection = re.split(
        "(\\\\subsection\\{(.*?)\\})", text_section, flags=re.MULTILINE | re.DOTALL
    )
...
with open(yaml_file, "w", encoding="utf-8") as out:
    for mytext in text_split:
        thesection = re.search(
            "\\\\section\\{(.*?)\\}", mytext, flags=re.MULTILINE | re.DOTALL
        )
        thesubsection = re.search(
            "\\\\subsection\\{(.*?)\\}", mytext, flags=re.MULTILINE | re.DOTALL
        )
        if thesection:

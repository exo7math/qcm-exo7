...
def split_section(title):
    ...
    for t in title_split:
        tt = re.sub('[,\.\s]','',t,flags=re.MULTILINE)
        if not tt.isdigit():
            ttt = re.sub('^[\s]*','',t,flags=re.MULTILINE) # Delete potential space at the begin
            ttt = re.sub('[\s]*$','',ttt,flags=re.MULTILINE) # Delete potential space at the end

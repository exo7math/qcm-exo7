...
parser = argparse.ArgumentParser(
    description="Conversion of mutliple choice questions from yaml to other format (LaTeX by default)."
)
parser.add_argument(
    "-f", "--format", nargs="?", default="tex", help="output format file"
)
...
if output:
    ...
    if output_format == "moodle":
        output_file = (
            file_name + ".xml"
        )  # Convert to a xml file with a.moodle extension (avec a.xml pour Moodle de ULille)
...
def split_section(title):
    ...
    for t in title_split:
        ...
        if not tt.isdigit():
            ttt = re.sub(
                r"^[\s]*", "", t, flags=re.MULTILINE
            )  # Delete potential space at the begin
            ttt = re.sub(
                r"[\s]*$", "", ttt, flags=re.MULTILINE
            )  # Delete potential space at the end
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        for data in all_data:
            ...
            for answer in data["answers"]:
                ...
                value = (
                    value.rstrip()
                )  # re.sub('[\s]*$','',text,flags=re.MULTILINE) # Delete potential space at the end
...
def replace_latex_macros(text):
    ...
    text = re.sub(
        r"\\Nn(?=[^a-zA-Z])", r"\\mathbf{N}", text, flags=re.MULTILINE | re.DOTALL
    )
    text = re.sub(
        r"\\Zz(?=[^a-zA-Z])", r"\\mathbf{Z}", text, flags=re.MULTILINE | re.DOTALL
    )
    text = re.sub(
        r"\\Qq(?=[^a-zA-Z])", r"\\mathbf{Q}", text, flags=re.MULTILINE | re.DOTALL
    )
    text = re.sub(
        r"\\Rr(?=[^a-zA-Z])", r"\\mathbf{R}", text, flags=re.MULTILINE | re.DOTALL
    )
    text = re.sub(
        r"\\Cc(?=[^a-zA-Z])", r"\\mathbf{C}", text, flags=re.MULTILINE | re.DOTALL
    )
    text = re.sub(
        r"\\Kk(?=[^a-zA-Z])", r"\\mathbf{K}", text, flags=re.MULTILINE | re.DOTALL
    )
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        for data in all_data:
            ...
            if "type" in data.keys() and (
                data["type"] == "onlyone" or data["type"] == "truefalse"
            ):
                out.write("\n\n\\begin{question}{" + myid + "}\n\n")
            else:
            ...
            elif "multicol" in data.keys() and data["multicol"]:
...
def encode_image(filename):
    ...
    data = (
        '</p><p>\n<img style="max-width:100%; margin: 10px auto;" src="data:image/png;base64,'
        + encoded
        + '"/>\n'
    )
    return data
...
def replace_images(data):
    ...
    while theimage is not None:
        ...
        theimage = re.search(
            r"(?<=\\qimage\{)(.*?)(?=\})", data, flags=re.MULTILINE | re.DOTALL
        )
        if theimage:
            ...
            data = re.sub(
                r"\\qimage\{(.*?)\}", new_image, data, flags=re.MULTILINE | re.DOTALL
            )
        else:
    ...
    with open(output_file, "w", encoding="utf-8") as out:
        ...
        for data in all_data:
            ...
            if "author" in data.keys():
                ...
                autotags += (
                    "    <tag><text>" + tagkeywd + "=" + tagval + "</text></tag>\n"
                )
            ...
            if "Logique" in thesection:
                ...
                ("Continuité" in thesection)
                or ("Limite" in thesection)
                or ("Ensembles" in thesection)
                or ("Fonctions" in thesection)
            ):
                tagval = "Fonctions"
            ...
            if ("type" not in data.keys()) or (
                "type" in data.keys() and (data["type"] != "numerical")
            ):
                if nbgood > 0:
                ...
                for answer in data["answers"]:
                    if answer["correct"] == "True":
                        out.write(
                            '<answer fraction="'
                            + goodratio
                            + '" format="html"><text><![CDATA[<p>\n'
                        )
                    else:
                        out.write(
                            '<answer fraction="0" format="html"><text><![CDATA[<p>\n'
                        )
                ...
                for answer in data["answers"]:
                    if answer["correct"]:
                        if "score" in answer:
                            ...
                            out.write(
                                '<answer format="moodle_auto_format" fraction="'
                                + myscore
                                + '"><text><![CDATA[\n'
                            )
                        else:
                            out.write(
                                '<answer format="moodle_auto_format" fraction="100"><text><![CDATA[\n'
                            )
                    else:
                        out.write(
                            '<answer format="moodle_auto_format" fraction="0"><text><![CDATA[\n'
                        )
...
def f2sxmlcleanup(data):
    ...
    data = re.sub(
        r"\\\[(.*?)\\\]", r'<sc:textLeaf role="mathtex">\1</sc:textLeaf>', data
    )
    data = re.sub(
        r"\\\((.*?)\\\)", r'<sc:textLeaf role="mathtex">\1</sc:textLeaf>', data
    )
    data = re.sub(
        r"\\textbf\{(.*?)\}", r'<sc:inlineStyle role="emp">\1</sc:inlineStyle>', data
    )
    return data
...
if output_format == "f2s":
    ...
    for data in all_data:
        ...
        thequestion = replace_latex_macros(data["question"])
        out.write(
            '<sc:question><op:res><sp:txt><op:txt><sc:para xml:space="preserve">\n'
        )
        ...
        for answer in data["answers"]:
            ...
            out.write(
                "<sc:choice"
                + checkstate
                + '><sc:choiceLabel><op:txt><sc:para xml:space="preserve">\n'
            )
            thevalue = replace_latex_macros(answer["value"])
            ...
            if "feedback" in answer:
                ...
                feedback = feedback.rstrip()
                out.write(
                    '<sc:choiceExplanation><op:txt><sc:para xml:space="preserve">\n'
                    + feedback
                    + "</sc:para></op:txt></sc:choiceExplanation>\n"
                )
        ...
        if "explanations" in data.keys():
            theexplanations = replace_latex_macros(data["explanations"])
            out.write(
                '<sc:globalExplanation><op:res><sp:txt><op:txt><sc:para xml:space="preserve">\n'
            )
